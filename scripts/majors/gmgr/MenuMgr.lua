...
function MenuMgr:Render()
	if Option.bMenuPlayed then 
		if Option.sGameState == "MENU" then -- 开始界面
			...
			local nTitleX = (graphicsWidth*0.5) - AssetsMgr:GetFont(722):getWidth(GameTextCfg.GetTextFunc(10000))*0.5;
			local nTitleY = graphicsHeight*0.35;
			love.graphics.print(GameTextCfg.GetTextFunc(10000), nTitleX,nTitleY);
			love.graphics.setColor(1,1,1,self.nCricleR);
			love.graphics.setFont(AssetsMgr:GetFont(242));
			local nStartBtnX = (graphicsWidth*0.5) - AssetsMgr:GetFont(242):getWidth(GameTextCfg.GetTextFunc(10001))*0.5;
			local nStartBtnY = graphicsHeight*0.6;
			love.graphics.print(GameTextCfg.GetTextFunc(10001),nStartBtnX,nStartBtnY);
